...
export default {
  ...
  methods: {
    ...
    goDetail(val) {
      //打一个标识说明我去了详情页,方便返回的时候还是这个页码
      sessionStorage.setItem("isGoDetail", true);
  ...
  created() {
    //不能在这里移除,只能在离开页面的时候才能移除
    // sessionStorage.removeItem("isGoDetail");
    //判断刚才是否进入过详情页
    if (sessionStorage.getItem("isGoDetail")) {
      this.currentPage =
        parseInt(sessionStorage.getItem("currentPage")) || this.currentPage;
      this.pageSize =
        parseInt(sessionStorage.getItem("currentPageSize")) || this.pageSize;
    }
    this.getTableData();
    this.getCategory();
  },
